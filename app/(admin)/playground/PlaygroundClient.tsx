      ...
      <Card>
        <CardContent>
          <div className="flex flex-wrap items-end gap-3 py-1">
            <div>
              <label className="block text-xs text-[#8888aa] mb-1">Giornata</label>
              <select
                value={matchdayId ?? ''}
                onChange={(e) => setMatchdayId(e.target.value || null)}
                className="rounded-md border border-[#2e2e42] bg-[#0a0a0f] px-3 py-1.5 text-sm text-white"
              >
                {matchdays.map((m) => (
            ...
            <label className="flex items-center gap-2 text-sm text-[#8888aa] mb-1.5">
              <input
                type="checkbox"
                checked={includeBR}
            ...
            <button
              ...
              className="rounded-md border border-[#2e2e42] bg-[#0a0a0f] px-3 py-1.5 text-sm text-[#8888aa] hover:text-white"
            >
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <Card>
            ...
            />
            <CardContent>
              <div className="mb-3 flex gap-1 border-b border-[#2e2e42]">
...
function ResultRulesForm({ rules, onChange, base }: { rules: ResultRulesConfig; onChange: (r: ResultRulesConfig) => void; base: ResultRulesConfig }) {
  ...
  return (
    <div className="space-y-3">
      <div>
        <p className="mb-1.5 text-xs font-medium text-[#8888aa]">Soglie gol</p>
          ...
          {rules.thresholds.map((t, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-2">
                <span className="w-12 text-xs text-[#55556a]">≥</span>
                <input
                  type="number"
                  step="0.1"
                  value={t.min}
                  onChange={(e) => updateThreshold(i, 'min', parseFloat(e.target.value) || 0)}
                  className={`w-24 rounded border bg-[#0a0a0f] px-2 py-1 text-sm text-white ${changed ? 'border-amber-500/50' : 'border-[#2e2e42]'}`}
                />
                ...
                <input
                  ...
                  className={`w-16 rounded border bg-[#0a0a0f] px-2 py-1 text-sm text-white ${changed ? 'border-amber-500/50' : 'border-[#2e2e42]'}`}
                />
      ...
      <div>
        <p className="mb-1.5 text-xs font-medium text-[#8888aa]">Smoothing (anti-fortuna)</p>
      ...
      <div>
        <p className="mb-1.5 text-xs font-medium text-[#8888aa]">Punti</p>
...
function NumberField({ label, value, base, step, onChange }: {
  ...
  return (
    <label className="block">
      <span className="block text-[11px] text-[#55556a] mb-0.5">{label}</span>
      <input
        type="number"
        step={step}
        value={value}
        ...
        className={`w-full rounded border bg-[#0a0a0f] px-2 py-1 text-sm text-white ${changed ? 'border-amber-500/50' : 'border-[#2e2e42]'}`}
      />
    </label>
  )
}
...
function CollapsibleCard({ title, defaultOpen = false, children }: { title: string; defaultOpen?: boolean; children: React.ReactNode }) {
  ...
  return (
    <Card>
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between border-b border-[#2e2e42] px-6 py-4"
      >
        <h2 className="text-sm font-semibold text-[#f0f0fa]">{title}</h2>
        <span className="text-[#55556a]">{open ? '−' : '+'}</span>
      </button>
  ...
  return (
    <button
      ...
      className={[
        ...
        active ? 'border-indigo-400 text-indigo-300' : 'border-transparent text-[#8888aa] hover:text-white',
...
function TeamScoresPanel({ teams, teamNameById }: { teams: SimulationResponse['team_scores']; teamNameById: Map<string, string> }) {
  ...
  return (
    <table className="w-full text-sm">
      <thead><tr className="text-left text-xs text-[#55556a]"><th className="py-1">Team</th><th>Tot.</th><th className="text-right">NV</th></tr></thead>
      <tbody>
        {teams.map((t, idx) => (
          <tr key={t.team_id} className="border-t border-[#2e2e42]">
            ...
            <td className="text-right text-[#55556a]">{t.nv_count}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
function FixturesPanel({ fixtures, teamNameById, emptyHint }: { fixtures: NonNullable<SimulationResponse['competition_results'][number]>['fixtures']; teamNameById: Map<string, string>; emptyHint: string }) {
  ...
  return (
    <div className="space-y-1">
      {fixtures.map((f) => (
        <div key={f.fixture_id} className="flex items-center justify-between rounded border border-[#2e2e42] px-3 py-2 text-sm">
          ...
          <span className="ml-2 w-16 text-right text-xs text-[#55556a] font-mono">{f.home_fantavoto.toFixed(1)}–{f.away_fantavoto.toFixed(1)}</span>
        </div>
      ))}
    </div>
  )
...
function BattleRoyalePanel({ fixtures, standings, teamNameById }: { fixtures: NonNullable<SimulationResponse['competition_results'][number]>['fixtures']; standings: NonNullable<SimulationResponse['competition_results'][number]>['standings']; teamNameById: Map<string, string> }) {
  ...
  return (
    <div className="space-y-3">
      <table className="w-full text-sm">
        <thead><tr className="text-left text-xs text-[#55556a]"><th className="py-1">Team</th><th>Pt</th><th>V/N/P</th><th>GF:GS</th><th className="text-right">DR</th></tr></thead>
        <tbody>
          {standings.map((s, idx) => (
            <tr key={s.team_id} className="border-t border-[#2e2e42]">
              ...
              <td className="text-[#8888aa] font-mono">{s.wins}/{s.draws}/{s.losses}</td>
              <td className="text-[#8888aa] font-mono">{s.goals_for}:{s.goals_against}</td>
              <td className="text-right text-[#8888aa] font-mono">{s.goal_difference > 0 ? '+' : ''}{s.goal_difference}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function Empty({ children }: { children: React.ReactNode }) {
  return <div className="py-6 text-center text-sm text-[#55556a]">{children}</div>
}
...
function SaveDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="w-full max-w-md rounded-xl border border-[#2e2e42] bg-[#0d0d14] p-6 shadow-2xl">
        {!done && !saving && (
          <>
            ...
            <div className="flex gap-3 justify-end">
              <button
                onClick={onClose}
                className="rounded-md border border-[#2e2e42] px-4 py-1.5 text-sm text-[#8888aa] hover:text-white"
              >
                Annulla
              </button>
              <button
        ...
        {saving && (
          <div className="py-6 text-center">
            ...
            <p className="text-sm text-[#8888aa]">Ricalcolo in corso…</p>
            <p className="mt-1 text-xs text-[#55556a]">Non chiudere questa finestra.</p>
          </div>
        )}
        ...
        {done && result && (
          <>
            ...
            <div className="mb-4 grid grid-cols-3 gap-2 text-center text-sm">
              <div className="rounded-lg border border-[#2e2e42] bg-[#111118] px-3 py-2">
                <div className="text-lg font-bold text-green-400">{result.matchdays_ok}</div>
                <div className="text-xs text-[#55556a]">Giornate OK</div>
              </div>
              <div className="rounded-lg border border-[#2e2e42] bg-[#111118] px-3 py-2">
                <div className="text-lg font-bold text-[#8888aa]">{result.matchdays_skipped}</div>
                <div className="text-xs text-[#55556a]">Saltate</div>
              </div>
              <div className="rounded-lg border border-[#2e2e42] bg-[#111118] px-3 py-2">
                <div className="text-lg font-bold text-red-400">{result.matchdays_errored}</div>
                <div className="text-xs text-[#55556a]">Errori</div>
              </div>
            </div>
