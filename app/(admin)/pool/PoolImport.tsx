      ...
      {STEPS.map((s, i) => (
        <div key={i} className="flex items-center">
          <div
            className={[
              ...
              i < current
                ...
                : 'border border-hairline bg-glass-1 text-ink-4',
            ].join(' ')}
          >
            {i < current ? '✓' : i + 1}
          </div>
          <span
            className={[
              'ml-2 text-xs',
              i === current ? 'text-ink-1' : 'text-ink-4',
            ].join(' ')}
          >
            {s.label}
          </span>
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full resize-y rounded-lg border border-hairline bg-glass-1 px-3 py-2 font-mono text-xs text-ink-1 placeholder-ink-4 outline-none focus:border-indigo-400/60/60 focus:ring-1 focus:ring-indigo-500/30"
            />
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between">
            <button
              type="button"
              onClick={() => setStep(0)}
              className="rounded-lg border border-hairline px-4 py-2 text-sm text-ink-3 hover:bg-glass-1 hover:text-ink-1 transition-colors"
            >
              ← Indietro
            </button>
            <button
      ...
      {step === 3 && parseResult && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
            {[
              { label: 'Totale', value: parseResult.stats.total, color: 'text-ink-1' },
              ...
              { label: 'Entrambi', value: parseResult.stats.matched_both, color: 'text-indigo-400' },
              {
                ...
                color: parseResult.stats.needs_roles > 0 ? 'text-amber-400' : 'text-ink-4',
              },
            ].map((s) => (
              <div
                key={s.label}
                className="rounded-lg border border-hairline bg-glass-1 px-3 py-2 text-center"
              >
                <div className={`text-2xl font-bold tabular-nums ${s.color}`}>{s.value}</div>
                <div className="mt-0.5 text-xs text-ink-4">{s.label}</div>
              </div>
            ))}
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-hairline text-left">
                    {['Nome', 'Squadra', 'Ruoli', 'Classe', 'SS', 'FM'].map((h) => (
                      <th
                        key={h}
                        className="px-3 py-2.5 text-xs font-medium uppercase tracking-wider text-ink-3"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-[#1e1e2a]">
                  {parseResult.preview.slice(0, 20).map((p, idx) => (
                    <tr key={idx} className="hover:bg-glass-1">
                      <td className="px-3 py-2 font-medium text-ink-1">{p.full_name}</td>
                      <td className="px-3 py-2 text-ink-3">{p.club || '—'}</td>
                      <td className="px-3 py-2">
                        {p.mantra_roles.length > 0 ? (
                          <span className="text-xs text-ink-1">{p.mantra_roles.join('/')}</span>
                        ) : (
          ...
          <div className="flex justify-between">
            ...
            {!importResult?.imported ? (
              ...
              <button
                ...
                className="rounded-lg bg-glass-1 border border-hairline px-4 py-2 text-sm text-ink-1 hover:bg-[#22222e] transition-colors"
              >
                Nuova importazione
              </button>
            )}
