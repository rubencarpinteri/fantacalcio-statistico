  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-bold text-ink-1">Audit Log</h1>
          <p className="text-sm text-ink-3">
            Registro delle operazioni · {ctx.league.name}
            {count != null && (
              <span className="ml-2 text-ink-4">({count.toLocaleString('it-IT')} voci totali)</span>
            )}
          </p>
        </div>
        ...
        <form method="GET" className="flex items-center gap-2">
          <select
            ...
            className="rounded-lg border border-hairline bg-glass-1 px-3 py-1.5 text-sm text-ink-1 focus:border-indigo-400/60 focus:outline-none"
          >
          ...
          {actionParam && (
            <a
              href="/audit"
              className="rounded-lg px-3 py-1.5 text-sm text-ink-4 hover:text-ink-1"
            >
              ✕
            </a>
          )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-hairline">
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-ink-4">Data/Ora</th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-ink-4">Utente</th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-ink-4">Azione</th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-ink-4">Entità</th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-ink-4">Dettagli</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-hairline">
                {logList.map((log) => {
                  ...
                  const actionColor = ACTION_COLOR[log.action_type as AuditAction] ?? 'text-ink-3'

                  return (
                    <tr key={log.id} className="hover:bg-glass-1">
                      <td className="px-4 py-2.5 text-ink-4 whitespace-nowrap text-xs">
                      ...
                      <td className="px-4 py-2.5 text-xs text-ink-4">
                        {ENTITY_LABEL[log.entity_type] ?? log.entity_type}
                        {log.entity_id && (
                          <span className="ml-1 font-mono text-[10px] text-ink-5">
                            {log.entity_id.slice(0, 8)}
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-2.5 max-w-xs truncate text-xs text-ink-4" title={detailStr ?? undefined}>
                        {detailStr ?? '—'}
                      </td>
                    </tr>
                  )
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <a
                ...
                className="rounded-lg border border-hairline px-3 py-1.5 text-ink-3 hover:bg-glass-1 hover:text-ink-1"
              >
                ← Precedente
              </a>
            )}
