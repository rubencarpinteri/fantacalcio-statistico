...
export default async function RoleRulesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-ink-1">Regole Ruoli Ambigui</h1>
                  ...
                  <div className="flex-1">
                    {existing ? (
                      <div className="flex items-center gap-3">
                        <p className="text-sm text-ink-3">Classificazione corrente:</p>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {ALL_MANTRA_ROLES.filter((r) => !AMBIGUOUS_ROLES.has(r)).map((role) => (
              <div
                key={role}
                className="flex items-center gap-1.5 rounded-lg border border-hairline bg-glass-1 px-3 py-1.5"
              >
                <span className="text-sm font-medium text-ink-1">{role}</span>
...
function InfoRow({ label, detail }: { label: string; detail: string }) {
  return (
    <div>
      ...
      <p className="mt-0.5 font-mono text-xs text-ink-4">{detail}</p>
    </div>
  )
}
