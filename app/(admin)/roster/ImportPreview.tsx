      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-ink-3">
                File CSV
              </label>
              <input
                ref={fileRef}
                ...
                className="block w-full text-sm text-ink-3 file:mr-4 file:rounded-lg file:border-0 file:bg-indigo-600 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-white hover:file:bg-indigo-700"
              />
            </div>
          ...
          <div className="rounded-lg border border-hairline bg-glass-1 p-4">
            <p className="mb-2 text-xs font-medium text-ink-3">Formato CSV atteso:</p>
            <pre className="text-xs text-ink-4">
{`Nome,Squadra,Ruolo
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-ink-1">Anteprima importazione</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-hairline text-left">
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-ink-3">#</th>
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-ink-3">Nome</th>
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-ink-3">Squadra</th>
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-ink-3">Ruoli</th>
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-ink-3">Rating Class</th>
                    <th className="px-4 py-3 text-xs font-medium uppercase tracking-wider text-ink-3">Stato</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#1e1e2a]">
                  {rows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-2.5 font-mono text-xs text-ink-4">
                          {row.rowIndex + 2}
                        </td>
                        <td className="px-4 py-2.5 text-ink-1">{row.full_name || '—'}</td>
                        <td className="px-4 py-2.5 text-ink-3">{row.club || '—'}</td>
                        ...
                        <td className="px-4 py-2.5">
                          {hasError ? (
                            ...
                            <select
                              ...
                              className={[
                                'rounded border px-2 py-1 text-xs bg-glass-1 focus:outline-none focus:border-indigo-400/60',
