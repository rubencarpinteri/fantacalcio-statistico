  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <a
          href={`/campionato/giornate/${matchdayId}/stats/import`}
          className="rounded-lg border border-hairline px-3 py-1.5 text-sm text-ink-3 hover:border-indigo-500/50 hover:text-ink-1 transition-colors"
        >
          Importa CSV
        </a>
        <label className="flex items-center gap-2 cursor-pointer text-sm text-ink-3 select-none">
          <input
            type="checkbox"
      ...
      {/* Tab bar */}
      <div className="flex gap-1 rounded-lg border border-hairline bg-transparent p-1 w-fit">
        {TABS.map((tab) => (
          <button
            key={tab.key}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-hairline">
                    <th className="sticky left-0 z-10 bg-transparent px-3 py-2 text-left text-xs font-medium text-ink-3 min-w-[160px]">
                      Giocatore
                    </th>
                    <th className="px-2 py-2 text-center text-xs font-medium text-ink-3 w-12">Min</th>
                    <th className="px-2 py-2 text-center text-xs font-medium text-ink-3 w-10">Prov</th>
      ...
      {filteredPlayers.length === 0 && (
        <p className="py-8 text-center text-sm text-ink-4">Nessun giocatore trovato con i filtri attivi.</p>
      )}
    </div>
  )
}
...
function Th({ children }: { children: React.ReactNode }) {
  return (
    <th className="px-1.5 py-2 text-center text-xs font-medium text-ink-3 min-w-[46px]">
      {children}
    </th>
  )
}
      ...
      <td className="sticky left-0 z-10 bg-transparent px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm text-ink-1 max-w-[140px]">{player.full_name}</p>
            <p className="text-xs text-ink-4">{player.club} · {player.mantra_roles.join('/')}</p>
          </div>
          {isInLineup && (
    ...
    <td className="px-1.5 py-2 text-center">
      <button
        ...
        className={[
          ...
          value ? trueClass : 'text-ink-4 hover:text-ink-3',
        ].join(' ')}
      >
        {label ?? (value ? '✓' : '—')}
      </button>
