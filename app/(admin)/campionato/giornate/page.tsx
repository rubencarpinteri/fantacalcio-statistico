          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-red-400/80">
              ...
              {openMatchdays.map((m, i) => (
                <span key={m.id}>
                  <a href={`/campionato/giornate/${m.id}`} className="underline hover:text-red-300 transition-colors">
                    {m.name}
                  </a>
        ...
        <div className="rounded-xl border border-hairline bg-glass-1 backdrop-blur-2xl overflow-hidden">
            ...
            <div>
              {prevMatchups.map((m, i) => {
                ...
                return (
                  <a
                    key={i}
                    href={prevMatchday ? `/campionato/giornate/${prevMatchday.id}/all-lineups` : '#'}
          ...
          {prevMatchday && (
            <div className="border-t border-hairline px-4 py-2">
              <a href={`/campionato/giornate/${prevMatchday.id}/results`} className="text-[11px] text-indigo-400 hover:text-indigo-300">
                Dettaglio giornata →
              </a>
            </div>
          )}
        ...
        <div className="rounded-xl border border-hairline bg-glass-1 backdrop-blur-2xl overflow-hidden">
          ...
          {nextMatchday && isAdmin && (
            <div className="border-t border-hairline px-4 py-2 flex items-center justify-between">
              <a href={`/campionato/giornate/${nextMatchday.id}`} className="text-[11px] text-indigo-400 hover:text-indigo-300">
                Gestione →
              </a>
                ...
                <div key={m.id} className={`px-4 py-2.5 ${isCurrent ? 'bg-indigo-500/5' : ''}`}>
                  ...
                  {isAdmin && isCurrent && (
                    <div className="mt-1.5 flex flex-wrap items-center gap-1 pl-7">
                      {isEditable && (
                        <a
                          href={`/campionato/giornate/${m.id}/import-lineups`}
