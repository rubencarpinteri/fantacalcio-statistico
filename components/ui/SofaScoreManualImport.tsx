  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-ink-3 uppercase tracking-wide">SofaScore (manuale)</span>
        {hasData && (
          <button onClick={handleClear} className="text-xs text-ink-4 hover:text-red-400">
            Cancella dati
          </button>
        )}
      </div>
      ...
      {hasData && (
        <p className="text-xs text-green-400">
          ...
          {lastAdded !== null && lastAdded > 0 && (
            <span className="text-ink-3"> (+{lastAdded} nuovi)</span>
          )}
          {lastAdded === 0 && (
            <span className="text-ink-4"> (nessun nuovo giocatore)</span>
          )}
        </p>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        {fixtures.map((fx) => (
          <a
            ...
            className="rounded border border-hairline px-2 py-1 text-xs text-indigo-400 hover:border-indigo-500 hover:text-indigo-300"
      ...
      <textarea
        ...
        className="w-full rounded-lg border border-hairline bg-glass-1 px-3 py-2 text-xs font-mono text-ink-1 placeholder-ink-4 focus:border-indigo-400/60 focus:outline-none resize-none"
      />
